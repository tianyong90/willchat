...
@section('main')
    <div class="portlet light">
        <div class="portlet-title">
            ...
            <div class="actions">
                <a href="{:U('exportExcel',array('token'=>$token,'cid'=>$_GET['cid']))}" class="btn default green-stripe btn-xs" target-form="ids"><i class="fa fa-file-excel-o "></i>&nbsp;导出Excel</a>
                <button url="{:U('setStatus',array('Model'=>'MemberCardMember','status'=>1))}" class="btn default green-stripe btn-xs ajax-post" target-form="ids"><i class="fa fa-check"></i>&nbsp;启用</button>
                <button url="{:U('setStatus',array('Model'=>'MemberCardMember','status'=>0))}" class="btn default yellow-stripe btn-xs ajax-post confirm" target-form="ids"><i class="fa fa-times"></i>&nbsp;禁用</button>
            ...
            <div class="inputs" style="padding:0;">
                <div class="portlet-input input-inline input-sm">
                    <div class="input-icon right">
                        ...
                        <input class="form-control" id="input-search" value="{$Think.get.keyword}" placeholder="输入卡号/姓名搜索" type="text">
                    </div>
                </div>
            </div>
        </div>
                ...
                <tbody>
                    <notempty name="list">
                        ...
                        <tr class="odd gradeX">
                            ...
                            <td>
                                <a class="btn blue btn-xs dialog-popup" href="{:U('LuckMoney/send',array('openid'=>$member['openid'],'token'=>$token))}">发送红包</a>
                                <button class="btn red btn-xs btn-delete-confirm" data-link="{:U('deleteMember',array('token'=>$token,'cid'=>$thisCard['id'],'mid'=>$member['mid']))}">删除</button>
                            </td>
                        </tr>
                        </volist>
                    <else/>
